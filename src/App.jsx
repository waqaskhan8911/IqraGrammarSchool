...
export default function SchoolApp() {
  ...
  // For adding arrears to a student in edit panel
  const [editAddArrearsAmount, setEditAddArrearsAmount] = useState('');
  const [editRemoveArrearsAmount, setEditRemoveArrearsAmount] = useState('');
...
const addToast1 = (message, { type = 'warning', duration = 1400 } = {}) => {
  ...
  // Calculate total pending for display: arrears + sum of extraFees pending
  const calculateTotalPendingForStudent = (s) => {
    const basePending = Number(s.arrears || 0);
  ...
  // Display arrears as '000' when zero, otherwise format as currency
  const formatArrearsDisplay = (n) => {
    return Number(n || 0) === 0 ? '000' : formatCurrency(n);
  };
  ...
  const removeGlobalExtraFee = (feeId) => {
    addConfirmToast('Remove this global fee from all students?', () => {
      ...
      // Keep editingStudent in sync if it references an extra from this global fee
      setEditingStudent(prev => {
        if (!prev) return prev;
        const updatedExtras = (prev.extraFees || []).filter(ef => ef.globalFeeId !== feeId);
        if (updatedExtras.length === (prev.extraFees || []).length) return prev; // no change
        return { ...prev, extraFees: updatedExtras };
      });
  ...
  // Auto-renew monthly fees on 21st of every month (only for unpaid students)
useEffect(() => {
  ...
  const lastRenewal = localStorage.getItem('lastFeeRenewDate');
  const lastDate = lastRenewal ? new Date(lastRenewal) : null;

  const alreadyRenewedThisMonth =
    lastDate &&
    lastDate.getMonth() === currentMonth &&
    lastDate.getFullYear() === currentYear;

  if (day === 23 && !alreadyRenewedThisMonth) {
    setStudentsData(prev => {
      const updated = { ...(prev || {}) };
      ...
      Object.keys(updated).forEach(classKey => {
        updated[classKey] = (updated[classKey] || []).map(student => {
          const s = { ...student };
          const feeHistory = Array.isArray(s.feeHistory) ? s.feeHistory : [];

          const thisMonth = new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
          const paidThisMonth = feeHistory.some(
            f => f.month === thisMonth && f.status.toLowerCase().includes('paid')
          );

          if (!paidThisMonth) {
            s.arrears = Number(s.arrears || 0) + Number(s.monthlyFee || 0);
            s.feeHistory = [...feeHistory, {
              month: thisMonth,
              amount: s.monthlyFee || 0,
              status: 'Pending',
              date: new Date().toLocaleDateString(),
              note: 'Monthly fee auto-renewed (unpaid student)'
            }];
          }
    ...
    localStorage.setItem('lastFeeRenewDate', today.toISOString());
    addToast('Monthly fees renewed for unpaid students only.', { type: 'success' });
  }
}, [setStudentsData]);
  ...
  const saveEditedStudent = () => {
    ...
    // const { classKey, id, name, rollNo, fatherName, monthlyFee, arrears, totalPaid, extraFees } = editingStudent;
     setEditingStudent(null);
     addToast('Editing panel closed.');
    if (!classKey) {
    ...
    setEditingStudent(null);
    addToast('Student saved successfully.');
  };

  // Admin: add arrears to the currently editing student
  const addArrearsToEditingStudent = () => {
    if (!editingStudent) {
      addToast('No student selected to add arrears.', { type: 'error' });
      return;
    }
    const amount = Number(editAddArrearsAmount || 0);
    if (isNaN(amount) || amount <= 0) {
      addToast('Please enter a valid arrears amount.', { type: 'error' });
      return;
    }
    ...
    setStudentsData(prev => {
      const copy = { ...(prev || {}) };
      if (!Array.isArray(copy[classKey])) return prev;
      const idx = copy[classKey].findIndex(s => s.id === editingStudent.id);
      if (idx === -1) return prev;

      const student = { ...copy[classKey][idx] };
      ...
      student.feeHistory.push({
        ...
        status: 'Pending',
        date: new Date().toLocaleDateString(),
        note: 'Manual arrears added by admin'
      });

      copy[classKey] = [...copy[classKey]];
      copy[classKey][idx] = student;
      return copy;
    });

    // update editingStudent shown in UI immediately
    setEditingStudent(prev => {
      if (!prev) return prev;
      const newFeeHistory = Array.isArray(prev.feeHistory) ? [...prev.feeHistory] : [];
      newFeeHistory.push({
        month: new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' }),
        amount,
        status: 'Pending',
      ...
      return { ...prev, arrears: (Number(prev.arrears || 0)) + amount, feeHistory: newFeeHistory };
    });

    setEditAddArrearsAmount('');
    addToast(`Added arrears Rs. ${amount.toLocaleString()}`);
  };

  // Admin: remove arrears amount from editing student (reduce arrears)
  const removeArrearsFromEditingStudent = () => {
    if (!editingStudent) {
      addToast('No student selected to remove arrears from.', { type: 'error' });
      return;
    }
    const amount = Number(editRemoveArrearsAmount || 0);
    if (isNaN(amount) || amount <= 0) {
      addToast('Please enter a valid amount to remove.', { type: 'error' });
      return;
    }
    ...
    setStudentsData(prev => {
      ...
      const student = { ...copy[classKey][idx] };
      student.arrears = Math.max(0, (Number(student.arrears || 0)) - amount);
      student.feeHistory = Array.isArray(student.feeHistory) ? [...student.feeHistory] : [];
      student.feeHistory.push({
        month: new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' }),
        amount: -amount,
        status: 'Removed',
        date: new Date().toLocaleDateString(),
        note: 'Manual arrears removed by admin'
      });
    ...
    // update editingStudent shown in UI
    setEditingStudent(prev => {
      if (!prev) return prev;
      const newFeeHistory = Array.isArray(prev.feeHistory) ? [...prev.feeHistory] : [];
      newFeeHistory.push({
      ...
      return { ...prev, arrears: Math.max(0, (Number(prev.arrears || 0)) - amount), feeHistory: newFeeHistory };
    });

    setEditRemoveArrearsAmount('');
    addToast(`Removed arrears Rs. ${amount.toLocaleString()}`);
  };
  ...
  const handlePayment = (e) => {
    ...
    // compute total pending (arrears + extras + monthly)
    const basePending = Number(selectedStudent.arrears || 0);
    const extrasPending = (selectedStudent.extraFees || []).reduce((a, ef) => a + (ef.status === 'pending' ? Number(ef.amount || 0) : 0), 0);
    const monthlyAmount = Number(selectedStudent.monthlyFee || 0);
    const totalPending = basePending + extrasPending + monthlyAmount;
    if (amountToPay > totalPending) {
      addToast('Payment cannot exceed total pending fee (arrears + extras + monthly).', { type: 'error' });
      return;
    }
    ...
    // 1) Pay arrears first
    const arrearsBefore = Number(studentRef.arrears || 0);
if (arrearsBefore > 0 && remaining > 0) {
  const payArrears = Math.min(arrearsBefore, remaining);
  studentRef.arrears = Math.max(0, arrearsBefore - payArrears);
  studentRef.totalPaid = (studentRef.totalPaid || 0) + payArrears;
  ...
  feeHistoryAdds.push({
    ...
    for (let i = 0; i < studentRef.extraFees.length && remaining > 0; i++) {
      ...
      if (ef.status === 'pending') {
        ...
        if (remaining >= efAmount) {
          ...
          // partial payment on an extra -> reduce amount
          const paidPortion = remaining;
          ef.amount = Number(efAmount - paidPortion);
    ...
    // 3) Allocate to current monthly fee
    if (monthlyAmount > 0 && remaining > 0) {
      const paidMonthly = Math.min(monthlyAmount, remaining);
      studentRef.totalPaid = (studentRef.totalPaid || 0) + paidMonthly;
      feeHistoryAdds.push({
        month: new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' }),
        amount: paidMonthly,
        status: paidMonthly >= monthlyAmount ? 'Paid' : 'Paid (partial)',
        date: new Date().toLocaleDateString(),
        note: 'Monthly fee'
      });
      remaining -= paidMonthly;

      const unpaidMonthly = monthlyAmount - paidMonthly;
      if (unpaidMonthly > 0) {
        studentRef.arrears = (studentRef.arrears || 0) + unpaidMonthly;
        feeHistoryAdds.push({
          month: new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' }),
          amount: unpaidMonthly,
          status: 'Pending',
          date: new Date().toLocaleDateString(),
          note: 'Monthly shortfall moved to arrears'
        });
      }
    }
...
<title>${escapeHtml(title)}</title>
  ...
  .fee-box {  display:flex ; justify-content:space-between ; align-items: center; border-radius: 6px; text-align: center;  }
  .fee-label { font-size: 15px;  }
  .fee-value { font-size: 15px; font-weight: bold;color:  }
 ...
 .extras { font-size:15px; }
  ...
  .std-inform { font-size: 18px; font-weight: bold; margin-top: 8px;  }
  .Pym-detail {  margin-top: 20px; }
  .extras .extra-item {
  display: flex;
...
<body>
${pages.map(page => {
  // compute due date string once for this page
  const nextMonth = new Date();
  nextMonth.setMonth(nextMonth.getMonth() + 1);
  const dueDateStr = `5 ${nextMonth.toLocaleDateString("en-US", { month: "long", year: "numeric" })}`;
  ...
  return `<div class="page page-break">
    ${page.map(s => {
      ...
      const basePending = Number(s.arrears || 0);
      const extras = (s.extraFees || []).map(ef => ({
        label: String(ef.label || ""),
        amount: Number(ef.amount || 0),
        status: String(ef.status || "")
      }));
      const extrasPending = extras.reduce((acc, ef) => acc + (ef.status === 'pending' ? ef.amount : 0), 0);
      const totalDue = monthlyFee + basePending + extrasPending;
      ...
      const extrasHtml = extras.length === 0
        ...
        : extras.map(ef => `
            <div class="extra-item">
              <div class="extra-label">${escapeHtml(ef.label)}</div>
              <div class="extra-price">${formatPrintCurrency(ef.amount)} ${ef.status === 'paid' ? '(Paid)' : ''}</div>
            </div>
          `).join('');

      // common receipt HTML for one copy (school/student) with copyLabel param
      const receiptHtml = (copyLabel) => `
        <div class="receipt">
          <img src="http://localhost:5173/IqraGrammarSchool/src/assets/IGPS.jpg" class="watermark" alt="Logo"/>
          <div class="content">
            <div class="copy-label">${copyLabel}</div>
            <div class="header">Iqra Grammar Public School</div>
            <div class="sub-header meta"><i><b>Branch:</b> Gulshan Hameed Colony opposite wensum college D.I.Khan</i></div>
            <div class="d-flex">
              <div class="school-no">Ph: 03365716844</div>
              <div class="meta">Due date • ${dueDateStr}</div>
            </div>
            <div class="std-inform">Student Information</div>
            ...
            <div class="d-flex">
              <div>
                ...
                <div class="fee-label">Arrears</div>
              </div>
              <div style="text-align:right;">
                <div>${formatPrintCurrency(monthlyFee)}</div>
                <div>${basePending === 0 ? '000' : formatPrintCurrency(basePending)}</div>
              </div>
            </div>
            ...
            <div style="margin-top:6px;" class="fee-box">
              <div><b>Total Due:</b></div>
              <div>${formatPrintCurrency(totalDue)}</div>
            </div>
          </div>
        </div>
      `;

      // each student-block contains two receipts (school copy + student copy)
      return `
        <div class="student-block">
          ${receiptHtml('School Copy')}
          ${receiptHtml('Student Copy')}
        </div>
      `;
    }).join('')}
  </div>`;
                ...
                <div className="bg-gradient-to-r from-blue-50 to-purple-50 p-8 rounded-lg mb-8">
                  ...
                  <div className="text-center mt-6">
                    <p className={`text-lg font-bold ${(calculateTotalPendingForStudent(selectedStudent) || 0) > 0 ? 'text-red-600' : 'text-green-600'}`}>
                      Status: {(calculateTotalPendingForStudent(selectedStudent) || 0) > 0 ? 'Fee Pending' : 'No Pending Fees'}
                    </p>
                  </div>
                </div>
              ...
              {editingStudent && (
                <div className="bg-yellow-50 p-4 mb-4 rounded">
                  ...
                  <div className="mt-3 flex gap-2">
                    ...
                    <button onClick={() => { saveEditedStudent(); addToast('Edit Successful', { type: 'success' }); }} className="bg-blue-600 text-white px-3 py-1 border rounded">Save</button>
                  </div>

                  {/* Add Arrears control (admin) */}
                  <div className="mt-3 flex gap-2 items-center">
                    <input
                      type="number"
                      value={editAddArrearsAmount}
                      onChange={(e) => setEditAddArrearsAmount(e.target.value)}
                      placeholder="Add Arrears"
                      className="p-2 border rounded flex-1"
                    />
                    <button onClick={addArrearsToEditingStudent} className="px-3 py-2 bg-red-600 text-white rounded">Add Arrears</button>
                  </div>
                  <div className="mt-2 flex gap-2 items-center">
                    <input
                      type="number"
                      value={editRemoveArrearsAmount}
                      onChange={(e) => setEditRemoveArrearsAmount(e.target.value)}
                      placeholder="Remove Arrears"
                      className="p-2 border rounded flex-1"
                    />
                    <button onClick={removeArrearsFromEditingStudent} className="px-3 py-2 bg-green-600 text-white rounded">Remove Arrears</button>
                  </div>
                </div>
              )}
